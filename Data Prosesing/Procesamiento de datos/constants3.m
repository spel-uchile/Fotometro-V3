...
%calibra las constandes del fotometro por dia, total y promedio
    ...
    while mn < length(aero.date)
        ...
        if ~isempty(index_stgo)
            ...
            mse_1 = @(l,v,d) sum(((l.*(log(v./(r1.^2))-log(stgo.sens1(valid1_pc))-reyleigh(408).*p1.*m1)./m1+d)-(ir1_440.*(l./hig_1).^(-ams_1))).^2);
            mse_2 = @(l,v,d) sum(((l.*(log(v./(r2.^2))-log(stgo.sens2(valid2_pc))-reyleigh(408).*p2.*m2)./m2+d)-(ir2_440.*(l./hig_2).^(-ams_2))).^2);
            mse_3 = @(l,v,d) sum(((l.*(log(v./(r3.^2))-log(stgo.sens3(valid3_pc))-reyleigh(408).*p3.*m3)./m3+d)-(ir3_440.*(l./hig_3).^(-ams_3))).^2);
            mse_4 = @(l,v,d) sum(((l.*(log(v./(r4.^2))-log(stgo.sens4(valid4_pc))-reyleigh(408).*p4.*m4)./m4+d)-(ir4_440.*(l./hig_4).^(-ams_4))).^2);         
            ...
            if ~isempty(valid1_pc)
                [x1, fval1] = opt2(mse_1, n1, 4095, 1,0);                
            else
                x1 = [4095; 1; 0];
                fval1 = 0;
            end
            ...
            if ~isempty(valid2_pc)
                [x2, fval2] = opt2(mse_2, n2, 4095, 1, 0);                
            else
                x2 = [4095; 1; 0];
                fval2 = 0;
            end
            ...
            if ~isempty(valid3_pc)
                [x3, fval3] = opt2(mse_3, n3, 4095, 1, 0);                
            else
                x3 = [4095; 1; 0];
                fval3 = 0;
            end
            ...
            if ~isempty(valid4_pc)
                [x4, fval4] = opt2(mse_4, n4, 4095, 1, 0);
            else
                x4 = [4095; 1; 0];
                fval4 = 0;
            end
    ...
    mse_1 = @(l,v,d) sum(((l.*(log(v./(r1.^2))-log(stgo.sens1(s_valid1))-reyleigh(408).*p1.*m1)./m1+d)-(ir1_440.*(l./hig_1).^(-ams_1))).^2);
    mse_2 = @(l,v,d) sum(((l.*(log(v./(r2.^2))-log(stgo.sens2(s_valid2))-reyleigh(408).*p2.*m2)./m2+d)-(ir2_440.*(l./hig_2).^(-ams_2))).^2);
    mse_3 = @(l,v,d) sum(((l.*(log(v./(r3.^2))-log(stgo.sens3(s_valid3))-reyleigh(408).*p3.*m3)./m3+d)-(ir3_440.*(l./hig_3).^(-ams_3))).^2);
    mse_4 = @(l,v,d) sum(((l.*(log(v./(r4.^2))-log(stgo.sens4(s_valid4))-reyleigh(408).*p4.*m4)./m4+d)-(ir4_440.*(l./hig_4).^(-ams_4))).^2);
