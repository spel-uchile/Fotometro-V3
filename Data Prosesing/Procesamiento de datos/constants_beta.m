function out = constants(aero, stgo, lt, ln, dig)
%calibra las constandes del fotometro por dia, total y promedio
        ...
        if ~isempty(index_stgo)
            ...
            if (x1(1) >= 395) && (x1(1) <= 425)
                ...
                for i = 1:1:length(valid1_pc)
                    ...
                    stg_valid1 = [stg_valid1 aod(stgo.sens1(valid1_pc(i)),r1(i),p1(i).*1013.25,m1(i),x1(2),x1(1))];
                end
            end
            if (x2(1) >= 395) && (x2(1) <= 425)
                ...
                for i = 1:1:length(valid2_pc)
                    ...
                    stg_valid2 = [stg_valid2 aod(stgo.sens2(valid2_pc(i)),r2(i),p2(i).*1013.25,m2(i),x2(2),x2(1))];                    
                end
            end
            if (x3(1) >= 395) && (x3(1) <= 425)
                ...
                for i = 1:1:length(valid3_pc)
                    ...
                    stg_valid3 = [stg_valid3 aod(stgo.sens3(valid3_pc(i)),r3(i),p3(i).*1013.25,m3(i),x3(2),x3(1))];
                end
            end
            if (x4(1) >= 395) && (x4(1) <= 425)
                ...
                for i = 1:1:length(valid4_pc)
                    ...
                    stg_valid4 = [stg_valid4 aod(stgo.sens4(valid4_pc(i)),r4(i),p4(i).*1013.25,m4(i),x4(2),x4(1))];
                end
            end            
    ...
    mse_1 = @(l,v) sum((((log(v./(r1.^2))-log(stgo.sens1(s_valid1))-reyleigh(l).*p1.*m1)./m1)-(ir1_440.*(l./hig_1).^(-ams_1))).^2);
    mse_2 = @(l,v) sum((((log(v./(r2.^2))-log(stgo.sens2(s_valid2))-reyleigh(l).*p2.*m2)./m2)-(ir2_440.*(l./hig_2).^(-ams_2))).^2);
    mse_3 = @(l,v) sum((((log(v./(r3.^2))-log(stgo.sens3(s_valid3))-reyleigh(l).*p3.*m3)./m3)-(ir3_440.*(l./hig_3).^(-ams_3))).^2);
    mse_4 = @(l,v) sum((((log(v./(r4.^2))-log(stgo.sens4(s_valid4))-reyleigh(l).*p4.*m4)./m4)-(ir4_440.*(l./hig_4).^(-ams_4))).^2);
    ...
    title('Scatterplot');
    hold off;
    saveas(fig1, strcat('sctr/',dig,'_s1_','.fig'));
    saveas(fig1, strcat('sctr/',dig,'_s1_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig2, strcat('sctr/',dig,'_s2_','.fig'));
    saveas(fig2, strcat('sctr/',dig,'_s2_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig3, strcat('sctr/',dig,'_s3_','.fig'));
    saveas(fig3, strcat('sctr/',dig,'_s3_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig4, strcat('sctr/',dig,'_s4_','.fig'));
    saveas(fig4, strcat('sctr/',dig,'_s4_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig1a, strcat('sctr/',dig,'_s1a_','.fig'));
    saveas(fig1a, strcat('sctr/',dig,'_s1a_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig2a, strcat('sctr/',dig,'_s2a_','.fig'));
    saveas(fig2a, strcat('sctr/',dig,'_s2a_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig3a, strcat('sctr/',dig,'_s3a_','.fig'));
    saveas(fig3a, strcat('sctr/',dig,'_s3a_','.png'));
    ...
    title('Scatterplot');
    hold off;
    saveas(fig4a, strcat('sctr/',dig,'_s4a_','.fig'));
    saveas(fig4a, strcat('sctr/',dig,'_s4a_','.png'));
